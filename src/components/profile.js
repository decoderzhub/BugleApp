...
import { Button, Text, StyleSheet, AsyncStorage } from "react-native";
...
import Main, { AppNavigator } from "../../app-navigator";
...
class ProfileScreen extends Component {
  ...
  _performSignOut = () => {
    ...
    this.props.dispatch(signedStatus(false));
    ...
    //   ? this.render(<MainNavigator />)
    //   : this.render(<AppNavigator />);
  };
  //pick image from library asynchronously
  _pickImage = async () => {
    // picks image from device
  ...
  _uploadImage = async (uri, imageName) => {
    ...
    var ref = firebase
      .storage()
      .ref()
      .child("images/" + this.props.auth.uid + "/" + imageName);
    await ref.put(blob);
    firebase
      .storage()
      .ref()
  ...
  render() {
    if (this.state.signedIn) {
      ...
      let follow =
        this.props.profile.profile_stats.following &&
        this.props.profile.profile_stats.following
          ? Object.keys(this.props.profile.profile_stats.following).length
          : 0;
      let posts = this.props.profile.profile_stats.posts
        ? Object.keys(this.props.profile.profile_stats.posts).length
        : 0;
      let received_request = this.props.profile.profile_stats.received_request
        ? Object.keys(this.props.profile.profile_stats.received_request).length
        : 0;
      let sent_request = this.props.profile.profile_stats.sent_request
        ? Object.keys(this.props.profile.profile_stats.sent_request).length
        : 0;
              ...
              <Col>
                <List>
                  ...
                  <ListItem
                    ...
                    onPress={() =>
                      this.props.navigation.navigate("ReceivedRequestScreen", {
                        received_request
                      })
                    }
                  />
